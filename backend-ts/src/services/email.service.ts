...
const PASSWORD_RESET_EMAIL_HTML = (displayName: string, link: string) => `
<!DOCTYPE html>
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#0A0A0C;" bgcolor="#0A0A0C">
    <tr>
      <td align="center" style="padding:48px 16px;background-color:#0A0A0C;" bgcolor="#0A0A0C">
        <table role="presentation" class="email-container" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;width:100%;">
          <tr>
            <td align="center" style="padding:0 0 40px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:28px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;">
              Drape
            </td>
          </tr>
          <tr>
            <td class="card" style="background-color:#161619;border-radius:20px;border:1px solid #222228;padding:44px 36px;" bgcolor="#161619">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding:0 0 28px;">
                    <table role="presentation" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:15px;line-height:24px;color:#9494AE;text-align:center;padding:0 0 32px;">
                    Hey ${displayName}, we received a request to reset your password.<br>
                    Tap the button below to choose a new one.
                  </td>
                </tr>
                <tr>
                  <td align="center" style="padding:0 0 28px;">
                    <a class="btn" href="${link}" style="display:inline-block;background-color:#7C3AED;color:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:15px;font-weight:600;line-height:1;text-align:center;padding:16px 44px;border-radius:28px;">
                      Reset my password
                    </a>
                  </td>
                </tr>
                <tr>
                  <td style="border-top:1px solid #222228;padding:20px 0 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:12px;line-height:18px;color:#6A6A82;text-align:center;word-break:break-all;">
                    If the button doesn't work, copy and paste this link into your browser:<br>
                    <a href="${link}" style="color:#A78BFA;">${link}</a>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding:28px 0 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;font-size:12px;line-height:18px;color:#6A6A82;text-align:center;">
...
export async function sendPasswordResetEmail(email: string, displayName?: string): Promise<{ messageId: string }> {
  if (!resend) {
    throw new Error('Resend API key not configured');
  }
  ...
  if (!hasPasswordProvider) {
    throw new Error('password-provider-not-enabled');
  }

  const link = await auth.generatePasswordResetLink(email);
  const safeDisplayName = escapeHtml((displayName || user.displayName || email.split('@')[0] || 'there').trim() || 'there');
  const safeLink = escapeHtml(link);
  ...
  const result = await resend.emails.send({
    ...
    to: email,
    ...(config.resendReplyTo ? { replyTo: config.resendReplyTo } : {}),
    subject: 'Reset your password — Drape',
    html: PASSWORD_RESET_EMAIL_HTML(safeDisplayName, safeLink),
  });
  ...
  log.info(`[Email] Password reset email queued for ${maskEmail(email)} (messageId: ${messageId})`);
  return { messageId };
}
