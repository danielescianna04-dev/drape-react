...
# ─── Branch guard — prod deploys ONLY from `main` ────────────────────────────
# Prevents accidentally pushing dev code to production. Override with FORCE=1
# (use only for hotfixes when you know what you're doing).
CURRENT_BRANCH="$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo '')"
if [[ "${FORCE:-0}" != "1" && "$CURRENT_BRANCH" != "main" ]]; then
  echo "❌ Aborting: prod deploys must run from the 'main' branch (you are on '$CURRENT_BRANCH')."
  echo "   → Merge your work to main first, or set FORCE=1 to override."
  exit 1
fi
if [[ -n "$(git status --porcelain)" ]]; then
  echo "⚠️  Warning: working tree has uncommitted changes — they WILL be deployed."
