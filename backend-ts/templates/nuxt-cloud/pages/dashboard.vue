...
async function fetchItems() {
  try {
    ...
    error.value = err?.data?.message || err?.message || 'Failed to load items'
  } finally {
    loading.value = false
  }
}
...
async function handleSubmit() {
  ...
  try {
    if (editingItem.value) {
      const updated = await $fetch<Item>(`/api/items/${editingItem.value.id}`, {
        method: 'PUT',
        body: formData.value,
      })
...
async function handleDelete(id: number) {
  try {
    ...
    error.value = err?.data?.message || err?.message || 'Failed to delete item'
  }
}
