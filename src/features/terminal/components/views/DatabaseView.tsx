...
interface DbState {
  ...
  supabaseUrl: string;
  /** Project uses the Bynot Cloud shared backend. Renders __bynot__ pseudo-db. */
  bynotCloudDetected: boolean;
...
type DbAction =
  ...
  | { type: 'SET_DATABASES'; databases: DbState['databases']; pgDetected: boolean; supabaseDetected?: boolean; supabaseUrl?: string; bynotCloudDetected?: boolean; containerReady: boolean }
  ...
  switch (action.type) {
    ...
    case 'SET_DATABASES':
      return {
        ...
        bynotCloudDetected: action.bynotCloudDetected || false,
...
export const DatabaseView: React.FC<Props> = ({ tab }) => {
  ...
  useEffect(() => {
    ...
    api.discover().then(data => {
      dispatch({ type: 'SET_DATABASES', databases: data.databases, pgDetected: data.pgDetected, supabaseDetected: data.supabaseDetected, supabaseUrl: data.supabaseUrl, bynotCloudDetected: data.bynotCloudDetected, containerReady: data.containerReady !== false });
    }).catch(() => {
  ...
  // Same thing for Bynot Cloud: if the project opted into the shared
  // backend, skip the discovery screen and jump straight to the table
  // list — there's only ever one "database" to show.
  useEffect(() => {
    if (state.bynotCloudDetected && !state.isLoading && state.screen === 'db-list') {
      const bynotDb = state.databases.find(d => d.path === '__bynot__');
      if (bynotDb && state.selectedDb !== '__bynot__') {
        dispatch({ type: 'SELECT_DB', dbPath: bynotDb.path });
      }
    }
  }, [state.bynotCloudDetected, state.databases, state.isLoading, state.screen, state.selectedDb]);
    ...
    <View style={styles.container}>
      ...
      {(state.screen === 'discovering' || state.screen === 'db-list') && (
        <DatabaseDiscovery
          ...
          bynotCloudDetected={state.bynotCloudDetected}
